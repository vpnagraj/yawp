...
Note that the install command above will install the package from GitHub *and* build vignettes. If you would prefer not to build vignettes upon install, then set `build_vignettes = FALSE` (default in `devtools::install_github()`).
...
- While `f1()` calcultes the F1 score, other available functions such `f_meas()` from [yardstick](https://CRAN.R-project.org/package=yardstick) may accommodate more generic applications (i.e. applying weights to precision and recall)
